...
export default class AuthController {
  ...
  async register(ctx : HttpContextContract){
    // TODO: Bugs doesn't fixed for wrong or invalid register 
  
    const check = await this.authValidator.validateSignupSchema(ctx)
  ...
  async login(ctx : HttpContextContract){
    ...
    return ctx.auth.use('web').attempt(data.uid, data.password)
  }
